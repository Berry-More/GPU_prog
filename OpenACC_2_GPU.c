...
#pragma acc kernels
    ...
    for (int j = 0; j < arraySize; j++)
    {
        ...
        outArray[j * arraySize] = 10.0 + 20.0 * j / (arraySize - 1);
        outArray[j * arraySize + arraySize - 1] = 20.0 + 20.0 * j / (arraySize - 1);
        outArray[arraySize * (arraySize - 1)+ j] = 30.0 + 10.0 * j / (arraySize - 1);
    }
}
...
#pragma acc kernels
    ...
    while (k < iterations & currentError > mainError)
    {
        ...
        #pragma acc loop collapse(2) reduction(max:currentError)
        for (int i = 1; i < sizeArray-1; i++)
        {
            for (int j = 1; j < sizeArray-1; j++)
            {
